...
│   └── style.css                # 样式文件
├── js/
│   ├── config.js                # 配置文件（含默认位置）
│   ├── app.js                   # 主应用逻辑（含经纬度显示）
│   ├── location.js              # 定位功能（含浏览器检测）
│   ├── map.js                   # 地图功能（含iframe分享）
│   ├── utils.js                 # 工具函数库
│   └── browser-detection.js     # 浏览器检测模块
├── test-*.html                   # 测试页面集合
│   ├── test-default-location.html    # 默认位置测试
│   ├── test-location-failure.html    # 定位失败测试
│   ├── test-iframe-share.html        # iframe分享测试
│   └── test-coordinates-display.html # 经纬度显示测试
├── share-map.html               # 地图分享页面（独立页面）
└── assets/
    ├── images/
    │   └── icons/               # 品牌Logo图标
    ...
    └── qrcodes/                 # 二维码图片
        ├── car001.png  # 雷克萨斯LS200 (浙***717)
        ├── car002.png  # 零跑T03 (浙***508)
        ├── car003.png  # 奥迪Q5 (浙***598)
        └── car004.png  # 小米SU7 (浙***779)
```
